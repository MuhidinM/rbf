...
const Experience = ({ cohortId }: { cohortId: number }) => {
  const [isOpen, setIsOpen] = useState(true);
  const [addNew, setAddNew] = useState("");
  // const [largestWeight, setLargestWeight] = useState<number>(0);
  const [experiences, setExperiences] = useState<AccountResponse[]>([]);
  const [experiencesDefault, setExperiencesDefault] = useState<
    AccountResponse[]
  >([]);
  const [experience, setExperience] = useState<AccountResponse>();
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await getAccountDurationById(
          `api/farmingExpriences/cohort/${cohortId}`
        );
        setExperiences(res);
        const res2 = await getAll("api/farmingExpriences/default");
        setExperiencesDefault(res2);
      } catch (error) {
        // @ts-ignore
        setError(error);
      } finally {
  ...
  const AddFromDefault = async (data: AccountResponse) => {
    try {
      setLoading(true);
      await createData("api/farmingExpriences", {
  ...
  // console.log("ExperiencesDefault", ExperiencesDefault);
      ...
      <div className="grid w-full gap-4">
        ...
        >
          <div className="flex items-center justify-between px-1 space-x-4">
            <h4 className="text-sm font-semibold">Default Experience</h4>
